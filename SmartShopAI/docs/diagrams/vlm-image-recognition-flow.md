...
- VLM 入口在 `vision_client.analyze_image_file_with_vlm`，默认走 Poe 的 OpenAI-compatible 接口；当前通过独立 `VLM_MODEL=GPT-4o` 配置识图模型，`VLM_API_KEY/VLM_BASE_URL` 留空时复用 `POE_API_KEY/POE_BASE_URL`。
